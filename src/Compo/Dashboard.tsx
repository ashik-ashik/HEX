...
import HexaHouseLoader from "./HexaHouseLoader";
...
const Dashboard: React.FC<DashboardProps> = ({
  ...
  const { readMealCount, bazarCosts, hexaDataLoader } =
    useAppData() as {
      readMealCount: MealCountResponse;
   ...
   * ============================================================
   *
   * Read directly from useAppData().
   *
   * If the first row is the sheet header, it is removed here
   * so the rest of the component can work with clean BazarItem[].
   */

  const bazarData = useMemo<BazarItem[]>(() => {
    const data = Array.isArray(bazarCosts?.data)
      ? bazarCosts.data
      : [];
  ...
  const mealDatesFromContext = useMemo(() => {
  ...
  const mealDataFromContext = useMemo<MealRow[]>(
    () => {
      const data = Array.isArray(
        readMealCount?.data
      )
      ...
      return houseMembers
        ...
        .map((member) => {
          const meals = data.map((entry) => {
            const value =
              entry?.meals?.[member?.email?.split("@")[0]];
    ...
    [readMealCount?.data, members]
  );

  const grandTotalMealsFromContext = useMemo(
    () => {
      return mealDataFromContext.reduce(
        (grandTotal, member) =>
          grandTotal + member.total,
        0
      );
    },
    [mealDataFromContext]
  ...
  const finalBalance =
    grandDeposit - totalBazar;

  const mealRate =
    grandTotalMealsFromContext > 0
      ? totalBazar /
        grandTotalMealsFromContext
      : 0;
      ...
      <div className="flex flex-col justify-center items-center h-screen bg-[#FAF5EB]">
        <div className="flex flex-col items-center gap-4">
          ...
          <div className="text-center">
            <p className="font-['Fraunces'] text-lg text-[#2B2117]">
              Opening the khata…
            </p>

            <p className="text-xs text-[#6B5E50] mt-1 font-['Inter']">
              Tallying deposits and bazar
              entries
            </p>
          </div>
        </div>
    ...
    return (
      <div className="p-6 bg-[#FAF5EB] min-h-screen flex flex-col justify-center items-center">
        <div className="text-center max-w-md border border-[#E4D9C5] bg-white rounded-xl px-8 py-10 shadow-sm">
          <div className="w-12 h-12 rounded-full bg-[#B23A2E]/10 text-[#B23A2E] flex items-center justify-center mx-auto mb-4 text-xl font-['Fraunces']">
            !
          </div>

          <h1 className="font-['Fraunces'] text-xl font-semibold text-[#2B2117] mb-2">
            The ledger didn't open
          </h1>

          <p className="text-[#6B5E50] text-sm font-['Inter']">
            Something went wrong while
            loading this month's data.
            Refresh the page, or check back
            in a moment.
          </p>
        </div>
      </div>
        ...
        <div className="min-h-screen pt-20 md:container mx-auto md:px-6 px-3 font-['Inter']">
          ...
          <div className="mb-10 pt-6">
            <div className="flex items-baseline justify-between flex-wrap gap-2 border-b-2 border-[#2B2117]/80 pb-3">
              <div>
                <p className="text-[10px] uppercase tracking-[0.2em] text-[#C0573B] font-semibold mb-1">
                  Mess Khata · Monthly
                  Statement
                </p>
              ...
              <p className="text-xs text-[#6B5E50] font-['JetBrains_Mono'] text-right">
                ...
                {bazarData.length} bazar entries
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 border border-[#E4D9C5] rounded-lg overflow-hidden bg-white mb-10 shadow-sm">
            {[
              ...
              >
                <p className="text-[10px] uppercase tracking-wider text-[#6B5E50] font-medium mb-2">
                  {card.label}
                </p>

                <p
                  className="font-['JetBrains_Mono'] text-lg font-semibold"
                  style={{
                    color: card.color,
                  }}
                >
          ...
          {insights && (
            <div className="grid sm:grid-cols-3 gap-3 mb-12">
              <div className="bg-[#B23A2E]/[0.06] border border-[#B23A2E]/20 rounded-lg p-4">
                <p className="text-[10px] uppercase tracking-wider text-[#B23A2E] font-semibold mb-1">
                  Owes the most
                </p>
              ...
              <div className="bg-[#4F7A5A]/[0.06] border border-[#4F7A5A]/20 rounded-lg p-4">
                <p className="text-[10px] uppercase tracking-wider text-[#4F7A5A] font-semibold mb-1">
                  Most in credit
                </p>
                ...
                <p className="font-['JetBrains_Mono'] text-sm text-[#4F7A5A] font-bold">
                  ৳{" "}
                  {insights.mostCredit.balance.toFixed(
                    0
                  )}{" "}
                  to receive
                </p>
              ...
              {insights.topBazarPerson && (
                <div className="bg-[#C28A2E]/[0.07] border border-[#C28A2E]/25 rounded-lg p-4">
                  <p className="text-[10px] uppercase tracking-wider text-[#C28A2E] font-semibold mb-1">
                    Top bazar contributor
                  </p>

                  <p className="font-['Fraunces'] text-base text-[#2B2117] capitalize">
                    {
                      showMemberName(insights
                        .topBazarPerson[0])?.name
                    }
                  </p>

                  <p className="font-['JetBrains_Mono'] text-sm text-[#C28A2E] font-bold">
                    ৳{" "}
                    {insights.topBazarPerson[1].toFixed(
                      0
                    )}{" "}
                    spent
                  </p>
                      ...
                      <div className="mt-3 pt-3 border-t border-dashed border-[#E4D9C5] flex flex-wrap gap-1.5">
                        {member.deposits.map(
                          (d, i) => (
                            <span
                              key={i}
                              className="text-[11px] font-['JetBrains_Mono'] bg-[#4F7A5A]/10 text-[#4F7A5A] px-1.5 py-0.5 rounded"
                            >
                              +{d}
                            </span>
                          )
              ...
              <table className="w-full text-sm">
                <thead className="bg-[#2B2117] sticky top-0 z-10">
                  <tr className="text-nowrap text-[11px] uppercase tracking-wide text-white">
                    ...
                    <th className="p-3 text-right font-medium text-white">
                      Amount (৳ )
                    </th>
                  </tr>
                </thead>
                ...
                <tbody>
                  {filteredBazar
                    ...
                    .map(
                      (
                        ...
                        >
                          <td className="p-3 text-[#6B5E50] font-['JetBrains_Mono']">
                            {item.date}
                          </td>

                          <td className="p-3 text-[#2B2117] capitalize">
                            {showMemberName(item.person)?.name}
                          </td>

                          <td className="p-3 text-right font-['JetBrains_Mono'] font-medium text-[#2B2117]">
                            ৳{" "}
                            {item.amount}
                          </td>
                        </tr>
                  ...
                  {filteredBazar.length ===
                    0 && (
                    <tr>
                      <td
                        colSpan={3}
                        className="p-6 text-center text-xs text-[#6B5E50]"
                      >
                        No bazar entries
                        match "
                        {bazarFilter}".
                ...
                <tfoot>
                  <tr className="border-t-2 border-[#2B2117]/80 bg-[#FAF5EB] sticky bottom-0">
                    <td className="p-3 font-semibold text-[#2B2117]">
                      Total Bazar
                    </td>

                    <td className="p-3"></td>

                    <td className="p-3 text-right font-['JetBrains_Mono'] font-semibold text-[#C0573B]">
                      ৳{" "}
                      {totalBazar.toFixed(
                        2
                      )}
          ...
          <section className="mb-12">
            <h2 className="font-['Fraunces'] text-xl font-semibold text-[#2B2117] mb-4 flex items-center gap-2">
              <span className="w-1.5 h-1.5 rounded-full bg-[#C0573B]" />
              Meal Counts{" "}
              <span className="text-sm text-[#6B5E50] font-['Inter'] font-normal">
                — {monthLabel}
              </span>
            </h2>
              ...
              <table className="min-w-max text-xs border-collapse w-full">
                <thead>
                  <tr className="text-nowrap text-[11px] uppercase tracking-wide">
                    <th className="p-3 sticky left-0 bg-[#2B2117] text-white z-10 text-left font-medium">
                      Name
                    </th>

                    <th className="p-3 bg-[#2B2117] text-white font-medium">
                      Total
                    </th>
                    ...
                    {mealDatesFromContext.map(
                      (date, idx) => (
                        <th
                          key={idx}
                          className="py-2 px-3 border-l border-[#E4D9C5] bg-[#FAF5EB] text-[#6B5E50] font-medium whitespace-nowrap"
                        >
                          {date}
                        </th>
                      )
                      ...
                      >
                        ...
                        {member.meals.map(
                          (meal, i) => (
                            <td
                              key={i}
                              className="p-2 border-l border-[#E4D9C5] text-[#2B2117] font-['JetBrains_Mono'] text-center"
                            >
                              {meal > 0 ? (
                                meal
                              ) : (
                                <span className="text-[#E4D9C5] block text-center flex items-center justify-center">
                                  <TbCircleFilled
                                    size={10}
                                    color="#ce3a00"
                                  />
            ...
            <div className="mt-4 inline-flex items-center gap-3 bg-white border border-[#E4D9C5] rounded-lg px-5 py-3">
              <p className="text-xs uppercase tracking-wide text-[#6B5E50] font-medium">
                Grand Total Meals
              </p>

              <p className="font-['Fraunces'] text-lg font-semibold text-[#2B2117]">
                {grandTotalMealsFromContext}
              </p>
            </div>
          </section>
          ...
          <section className="pb-16">
            ...
            <div className="grid sm:grid-cols-3 gap-3 mb-6">
              <div className="bg-white border border-[#E4D9C5] rounded-lg p-4 text-center">
                <p className="text-[10px] uppercase tracking-wider text-[#6B5E50] mb-1">
                  Total Bazar
                </p>

                <p className="font-['JetBrains_Mono'] text-base font-semibold text-[#C0573B]">
                  ৳{" "}
                  {totalBazar.toFixed(
                    2
                  )}
              ...
              <div className="bg-white border border-[#E4D9C5] rounded-lg p-4 text-center">
                ...
                <p className="font-['JetBrains_Mono'] text-base font-semibold text-[#2B2117]">
                  {
                    grandTotalMealsFromContext
                  }
                </p>
              ...
              <div className="bg-white border border-[#E4D9C5] rounded-lg p-4 text-center">
                ...
                <p className="font-['JetBrains_Mono'] text-base font-semibold text-[#C28A2E]">
                  ৳{" "}
                  {mealRate.toFixed(
                    4
                  )}
                      ...
                      {[
                        ...
                        (col, i) => (
                          <th
                            ...
                            className={`p-4 font-medium ${col.align} ${
                              col.key
                                ? "cursor-pointer select-none hover:text-[#2B2117]"
                                : ""
                            } ${
                              i === 0
                    ...
                    {sortedSettlements?.map(
                      (item, idx) => (
                        ...
                        >
                          ...
                          <td className="p-4 text-right font-['JetBrains_Mono'] text-[#2B2117]">
                            ৳{" "}
                            {item.mealCost.toFixed(
                              2
                            )}
                          </td>

                          <td
                            className={`p-4 text-right font-['JetBrains_Mono'] font-semibold ${
                              item.balance >=
                              0
                                ? "text-[#4F7A5A]"
                                : "text-[#B23A2E]"
                          ...
                          <td className="p-4 text-center">
                            {item.balance >
                            0 ? (
                              <span className="text-[11px] bg-[#4F7A5A]/10 text-[#4F7A5A] px-2.5 py-1 rounded-full font-medium">
                                Will Receive
                              </span>
                            ) : item.balance <
                              0 ? (
                              <span className="text-[11px] bg-[#B23A2E]/10 text-[#B23A2E] px-2.5 py-1 rounded-full font-medium">
                                Needs to Pay
                              </span>
                            ) : (
                              <span className="text-[11px] bg-[#2B2117]/5 text-[#6B5E50] px-2.5 py-1 rounded-full font-medium">
                                Settled
                              </span>
                            )}
                          </td>
